...
import {
  ...
  fetchTutorAssignmentSubmissions,
  fetchTutorAssignments,
  gradeTutorAssignmentSubmission,
  markAllTutorAssignmentNotificationsAsRead,
...
const TutorAssignments = () => {
  ...
  const loadNotifications = async () => {
    ...
    try {
      const payload = await fetchTutorAssignmentNotifications();
  ...
  useEffect(() => {
    ...
    const loadSubmissions = async () => {
      ...
      try {
        const payload = await fetchTutorAssignmentSubmissions(selectedAssignmentId);
        if (!cancelled) {
  ...
  const handleSaveAssignment = async () => {
    ...
    try {
      ...
      if (editorMode === 'edit' && editingAssignmentId) {
        const response = await updateTutorAssignment(editingAssignmentId, payload);
        setAssignments(response.assignments);
        setSelectedAssignmentId(response.assignment.id);
        setNotice(response.message);
        nextSelectedId = response.assignment.id;
      } else {
        const response = await createTutorAssignment(payload);
        setAssignments(response.assignments);
        setSelectedAssignmentId(response.assignment.id);
        setNotice(response.message);
        nextSelectedId = response.assignment.id;
  ...
  const handleDeleteAssignment = async (assignment: SchoolAssignment) => {
    ...
    try {
      const response = await deleteTutorAssignment(assignment.id);
      setAssignments(response.assignments);
  ...
  const openAssignmentFromNotification = async (notification: SchoolAssignmentNotification) => {
    ...
    if (!notification.isRead) {
      ...
      try {
        await markTutorAssignmentNotificationAsRead(notification.id);
  ...
  const handleMarkNotificationRead = async (notificationId: string) => {
    ...
    try {
      await markTutorAssignmentNotificationAsRead(notificationId);
      setNotifications((previous) =>
  ...
  const handleArchiveNotification = async (notificationId: string) => {
    setActiveNotificationId(notificationId);
    try {
      await archiveTutorAssignmentNotification(notificationId);
  ...
  const handleMarkAllNotificationsRead = async () => {
    ...
    try {
      await markAllTutorAssignmentNotificationsAsRead();
