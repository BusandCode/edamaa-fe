...
if [ "$api_health_code" = "200" ]; then
  ...
  if [ "$compat_code" != "404" ] && [ "$compat_post_code" != "404" ] && [ "$compat_exams_code" != "404" ] && [ "$compat_assignments_code" != "404" ] && [ "$compat_tutor_assignments_code" != "404" ] && [ "$compat_free_library_recommendations_code" != "404" ]; then
    echo "API already running: http://${API_HOST}:${API_PORT}"
    exit 0
  fi

  echo "API is running but missing one or more compatibility routes."
  echo "GET ${API_COMPAT_PATH} -> ${compat_code}, POST ${API_COMPAT_POST_PATH} -> ${compat_post_code}, GET ${API_COMPAT_EXAMS_PATH} -> ${compat_exams_code}, GET ${API_COMPAT_ASSIGNMENTS_PATH} -> ${compat_assignments_code}, GET ${API_COMPAT_TUTOR_ASSIGNMENTS_PATH} -> ${compat_tutor_assignments_code}, GET ${API_COMPAT_FREE_LIBRARY_RECOMMENDATIONS_PATH} -> ${compat_free_library_recommendations_code}"
