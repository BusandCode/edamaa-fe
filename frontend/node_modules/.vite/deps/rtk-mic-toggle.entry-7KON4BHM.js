...
var RtkMicToggle = class {
  ...
  render() {
    if (!this.meeting)
      return null;
    if (!this.canProduceAudio || ["OFF_STAGE", "REQUESTED_TO_JOIN_STAGE"].includes(this.stageStatus)) {
      return h(Host, { "data-hidden": true });
    }
    const { tooltipLabel, label, icon, classList, showWarning, disable } = this.getState();
    return h(Host, { title: label }, h("rtk-tooltip", { kind: "block", label: tooltipLabel, part: "tooltip" }, h("rtk-controlbar-button", { part: "controlbar-button", icon, label, size: this.size, iconPack: this.iconPack, variant: this.variant, class: classList, onClick: this.toggleMic, showWarning, disabled: disable })));
  }
  static get watchers() {
    return {
...
export {
  RtkMicToggle as rtk_mic_toggle
};
//# sourceMappingURL=rtk-mic-toggle.entry-7KON4BHM.js.map
